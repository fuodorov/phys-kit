...
%% формат А4 (60x84 1/8), 210x297 мм; полоса набора 170x257 мм;
%% поля по 20 мм; Times New Roman 12 пт, одинарный интервал,
%% абзац 10 мм, колонцифра внизу по центру, автоматические переносы,
%% выравнивание по ширине.
%% ---------------------------------------------------------------

%% Нижнее поле берём 20 мм плюс колонцифра в footskip.
...
%% таблицы данных набираем мельче — широкие выгрузки pandas
%% и в полосу 170 мм иначе не помещаются
...
%% код набираем мелко и с переносом длинных строк: в листингах
%% попадаются строки длиннее полосы набора
...
\setlength{\parindent}{0pt}
{\small
% УДК/ББК и авторский знак присваивает библиотека НГУ — так прямо сказано
% в образце выходных данных ИПЦ («индексы взять в библиотеке НГУ»).
% Проставленные здесь значения обоснованы, но подлежат подтверждению:
%   004.4     — программные средства (родитель для 004.41 «программотехника»)
%   :53       — знак отношения к физике
%   (075.8)   — учебное пособие для высшей школы
%   32.973-018 — вычислительная техника, программирование
%   я73       — учебное издание для вузов
%   Ф33       — авторский знак по двузначной таблице Хавкиной для «Федоров»
%               (для написания «Фёдоров» таблица даёт Ф31)
УДК~004.4:53(075.8)\\
ББК~32.973-018я73\\
Ф33
\vspace{6mm}

% Две строки оставлены на случай, если ИПЦ потребует двух рецензентов:
...
% Если хватит одного — заменить \textit{Рецензенты} на \textit{Рецензент}
...
\textbf{Федоров, В.\,В.}\\
Ф33\quad Разработка и применение программного обеспечения в физических
...
Предназначено для студентов направления <<Прикладная математика
и физика>>, изучающих дисциплину <<Применение Python в физических
исследованиях>>, и для всех, кто пишет программы в научной
работе.
\vspace{4mm}
...
\clearpage
% Объём блока должен быть чётным: у листа две стороны. Выходные данные
% ставим последней страницей, поэтому, если они пришлись бы на нечётную,
% доливаем перед ними одну пустую.
