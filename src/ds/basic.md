...
Терминология, сложившаяся в машинном обучении стихийно, сбивает с толку: одно и то же называют по-разному, а за привычными словами вроде «модель» или «обучение», заимствованными из обихода, стоит не совсем то, что в физике.

Для физика материал знаком под другими названиями. Задача обучения модели сводится к задаче оптимизации, функция потерь к минимизируемому функционалу, а регуляризация к способу сделать некорректную задачу корректной.
...
Задачи ML делят по тому, есть ли у объектов разметка (метка `y`), проставленная человеком, и какая она.
...
Разметки `y` нет или она не используется. Алгоритм ищет структуры, закономерности и связи в самих данных.
...
*   **Особенность:** заранее проставленных меток нет (обучение без учителя).
...
От типа признака зависит и выбор модели, и способ предобработки, применённой к данным.
...
Типы можно и преобразовывать. Разбей числовой признак на интервалы с заранее выбранными границами, и получишь порядковый.
...
Внутренние настройки модели называют **параметрами**, а их подбор по данным — **обучением**. Параметры двигают так, чтобы предсказания на обучающей выборке как можно ближе сходились с заранее проставленными метками (или, если меток нет, чтобы модель выявила скрытую структуру).
