...
Здесь речь о том, что не требует ничего, кроме самого языка; компиляторы и векторизация будут в следующей главе. Выигрыш обычно скромнее, зато цена нулевая: код остаётся обычным Python, который прочитает любой коллега.
...
Речь пойдёт об уровнях 3–5, хотя у первых двух потенциал ускорения наибольший, и цена ошибки тоже: переделывать выбранную архитектуру посреди работы дорого.

Оптимизировать можно не только скорость, но и память, место на диске и число обращений к нему, сетевой трафик, потребление энергии; здесь разберём только скорость и память.

За оптимизацию всегда приходится платить.

1. Она отнимает время без гарантии, что потраченные часы что-то дадут.
...
Третий уровень, реализация: дюжина советов, каждый с замером, сделанным на одной машине. Выигрыш дают не все.
...
Пример игрушечный, но если нужное уже есть в `builtins`, почти всегда быстрее взять готовое: встроенные функции, написанные на C, обходятся без цикла на уровне интерпретатора.

### Совет 2. Правильная фильтрация

Отберём из списка нечётные элементы, по предыдущему совету взяв встроенный `filter`.
...
`builtins` и генераторы не ускоряют код сами по себе: стоило заменить `lambda` на метод `str.isdigit`, написанный на C, и `filter` выиграл. Проверяй свой случай замером.

### Совет 3. Правильная проверка вхождений

> Разница между `in` по списку и по множеству разобрана в главе [«Сложность операций с коллекциями»](../dev/python/o-notation.md). Здесь — цена построения множества и расход памяти.
...
Данные взяты случайные, и это важно: на уже отсортированном списке Timsort вырождается в один линейный проход, оба замера падают примерно до 13 мс, и разницы между ними не остаётся. Копируют оба: `sorted` делает копию внутри себя, а второму варианту её делает срез, и стоит она около 2 мс. На случайных данных `sort` выходит вперёд на десяток процентов, так что, если исходный порядок не нужен, пользуйся методом, работающим на месте.
...
Самым быстрым оказался и самый идиоматичный вариант `if not elements`, рекомендованный любым руководством по стилю. Так бывает нечасто.
...
Предположим теперь, что у объектов, попадающих в функцию, нужного атрибута в основном нет.
...
Исключение, брошенное один раз, обходится дёшево, а миллион раз подряд — дорого. Выбирай между `hasattr` и `try/except` по тому, какая ситуация встречается чаще.
...
Разница берётся из обращения к имени: интерпретатору нужно выяснить, на что указывает `list`, тогда как литерал компилируется в одну инструкцию. Байт-код, разобранный модулем `dis`, это показывает.
...
Если функцию можно не вызывать, лучше не вызывать: на каждый вызов создаётся кадр стека, и время на него заметно.
...
Уловки ниже ухудшают читаемость кода ради нескольких процентов; применяй их, только если профилировщик показал, что эти проценты нужны.
...
Объявление в одну строку быстрее: распаковка кортежа обходится дешевле десяти отдельных присваиваний. Но читать такую строку невозможно, и выигрыш в сотню микросекунд того не стоит.

### Совет 12. Поиск функций и атрибутов

Поиск атрибута в Python не бесплатен: за ним стоит `__getattribute__`, а если тот ничего не нашёл, то и `__getattr__`. Напрашивается найти атрибут один раз и сохранить в локальную переменную, не разыскивая его заново на каждой итерации.
...
Совет, годами кочующий по подборкам об оптимизации, проигрывает: начиная с версии 3.11 CPython специализирует вызов метода в байт-коде, и обычный `result.append(item)` оказывается быстрее заранее сохранённой ссылки.
...
Замерять память в Python трудно, и первый способ, подсказанный документацией, обманет.
...
Кажется, всё работает как надо. Но присмотрись к числам.
...
Список объектов `SomeClass` занимает столько же, сколько список целых чисел: `sys.getsizeof` меряет размер самого списка, то есть массива указателей, а не то, на что эти указатели ведут, и надёжно работает только для простых типов и встроенных структур, размещённых в непрерывном куске памяти.
...
Этот подход тоже не идеален: он смотрит на потребление памяти процессом в отдельные моменты времени, учитывает не всё, а результаты заметно плавают от запуска к запуску.
...
В смысле C++ утечек в Python почти нет: за освобождением следит сборщик мусора, и потерять память можно разве что напортив со счётчиком ссылок в расширении, написанном на C. Подробнее об этом рассказано в [разборе устройства сборщика](https://rushter.com/blog/python-garbage-collector/).

Долгоживущие бесполезные объекты получить легко, и на практике утечкой обычно называют их. Классических способов три: изменяемый аргумент по умолчанию, забытая переменная, живущая всё время работы длинной функции, и заведённый на атрибуте класса кеш, из которого ничего не удаляется.
...
В старых версиях Python (2.7 и всё до 3.4) сборщик не умел разбирать циклические ссылки между объектами с `__del__`, и образованные ими циклы жили до конца работы программы.
...
`np.array` устроен так же, с фиксированным типом и уложенными подряд элементами, и памяти занимает существенно меньше стандартного списка. В отличие от `array`, он умеет считать.
...
Здесь `%memit` не годится: он меряет
прирост потребления процессом, а интерпретатор с аллокатором держат уже освобождённые
страницы про запас и кладут в них вновь созданный массив. Тогда `%memit`
...
Кортеж чуть компактнее списка: ему не нужен запас под рост. Разница невелика, зато видно, во что обходится списковое включение, создающее список с запасом места под будущие `append`.
...
Здесь разница ушла в шум: современный CPython кеширует поиск атрибута и в обычном `__dict__`.

С наследованием `__slots__` неудобен: указывать его приходится в каждом классе иерархии, иначе `__dict__` вернётся и экономия пропадёт.
...
Плата — время: упакованный флаг надо вынуть из байта.

### `range` и вычисление вместо хранения

Иногда последовательность не нужно хранить: `range` не держит элементы в памяти, а вычисляет нужный по индексу, и `len` тоже считает по формуле.
...
Для разбирательств с памятью пригодятся ещё два инструмента, работающих с живой кучей.
...
* Оптимизировать имеет смысл только измеренное: интуиция о том, где программа проводит время, врёт систематически, поэтому сначала профилировщик, потом правки.
* Оптимизация всегда чего-то стоит, отнимая время разработки, читаемость кода, иногда и корректность, поэтому, прежде чем ускорять, убедись, что медленная работа составляет проблему.
