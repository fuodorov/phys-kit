...
Работоспособность зеркала следует проверить сразу: выключить машину, отсоединить один диск, убедиться, что система загружается, затем вернуть его на место и повторить проверку со вторым диском.
...
В выводе `efibootmgr -v` должна появиться запись GRUB. Если она отсутствует, следует убедиться, что раздел с `.efi`-файлом смонтирован в `/boot` и отформатирован в FAT32.
