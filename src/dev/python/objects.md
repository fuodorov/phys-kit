...
> 💡 **Золотое правило**: для сравнения значений следует использовать `==`, а `is` применять только для проверки на `None`, `True` и `False`.
...
> 💡 **Совет**: для частых операций конкатенации следует использовать `list` + `join()` для строк или `bytearray` для байтов.
