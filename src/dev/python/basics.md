...
Python — высокоуровневый мультипарадигменный язык программирования с динамической типизацией; код на нём часто сравнивают с псевдокодом: мощные идеи выражаются в нескольких строках и легко читаются.

Прочитай [PEP 8](https://www.python.org/dev/peps/pep-0008/).
...
Целые и вещественные числа работают как в других языках.
...
В отличие от многих языков, в Python нет унарных операторов инкремента (x++) и декремента (x--). Целые здесь неограниченной точности — отдельного типа для длинных целых, как в Python 2, нет, `int` растёт, пока хватает памяти, — и есть встроенный тип комплексных чисел; подробности в [документации](https://docs.python.org/3/library/stdtypes.html#numeric-types-int-float-complex).
...
Так строки собирали до версии 3.6, и в чужом коде этот стиль ещё встречается, но новое пишут f-строками: перед кавычками ставится буква `f`, а всё, что стоит внутри строки в фигурных скобках, вычисляется и подставляется на место.
...
У строк много полезных методов.
...
Список всех строковых методов — в [документации](https://docs.python.org/3/library/stdtypes.html#string-methods).
...
Список — аналог массива, но его размер можно менять, а элементы могут быть разных типов.
...
Подробности о списках — в [документации](https://docs.python.org/3/tutorial/datastructures.html#more-on-lists).

### Срезы

Кроме доступа к элементам по одному, в Python есть синтаксис для работы с подсписками — срезы (slicing).
...
Если в теле цикла нужен ещё и индекс элемента, используй встроенную функцию `enumerate`.
...
Часто нужно преобразовать данные одного вида в другой. Простой пример — код, вычисляющий квадраты чисел.
...
То же самое списковым включением (list comprehension).
...
Словарь хранит пары (ключ, значение), примерно как `Map` в Java или объект в Javascript.
...
Подробности о словарях — в [документации](https://docs.python.org/3/library/stdtypes.html#dict). Итерация по ключам словаря:
...
Словарные включения (dictionary comprehensions) похожи на списковые, но строят словари.
...
Множество — неупорядоченная коллекция различных элементов.
...
_Циклы_. Итерация по множеству синтаксически не отличается от итерации по списку; но множества неупорядочены, и на порядок обхода полагаться нельзя.
...
Кортеж — неизменяемый упорядоченный список значений, во многом похожий на обычный список. Одно из важнейших отличий: кортежи можно использовать как ключи в словарях и как элементы множеств, а списки нельзя.
...
Как объекты устроены внутри и во что обходится каждая операция, разбирается в основных главах — [«Объекты и память»](./objects.md) и [«Классы»](./classes.md).
