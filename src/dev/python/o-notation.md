...
В настоящей главе рассматривается стоимость каждой операции над коллекциями Python и выбор структуры, соответствующей задаче.
...
Алгоритм проверяется на данных, приближенных к реальным. На неудачно подобранном наборе квадратичный код выглядит корректным, а проблема проявляется на реальных данных.
...
С версии Python 3.7 словарь сохраняет порядок вставки. Поиск по *значению* словарь не поддерживает: такой поиск стоит \\(O(n)\\). Если он требуется часто, следует завести второй словарь с обратным отображением.

### Оптимизация на практике

Если по коллекции многократно выполняется поиск, её следует преобразовать в множество или словарь один раз до цикла. Построение множества стоит \\( O(m) \\), столько же, сколько один линейный поиск, поэтому оно окупается уже на второй итерации.
