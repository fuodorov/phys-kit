...
Позиционные аргументы следуют первыми. Запись `min(x=-5, 12)` недопустима: интерпретатор не сможет определить, к какому параметру относится `12`.
...
Значение по умолчанию вычисляется один раз, в момент, когда интерпретатор читает строку `def`, а не при каждом вызове. Для числа или строки это незаметно, но изменяемый объект, созданный однажды, окажется общим для всех вызовов и будет накапливать состояние между ними.
...
Второй вызов на тех же данных возвращает пустой список, поскольку множество `seen` уже содержит все числа с предыдущего вызова. В объявлении указывается `None`, а настоящее пустое множество создаётся внутри функции при каждом вызове.
...
Можно потребовать, чтобы некоторые аргументы передавались только по имени.
...
Звёздочка слева от знака равенства собирает «всё остальное» в список. Имя, помеченное ею, может располагаться в начале, в конце или в середине; Python сначала распределяет значения по обычным именам, а остаток, не разобранный ими, достаётся звёздочке.
...
Здесь три разных `min`; `g` печатает свой, локальный. Если из `g` удалить строку с присваиванием, печататься начнёт `min`, объявленный в `f`; если удалить её и оттуда, будет напечатана глобальная 42. Поэтому переменные не называют именами встроенных функций: встроенный `min`, последнее звено цепочки, оказывается закрытым.
...
Функция `make_adder` давно завершилась, её локальные переменные должны были исчезнуть, но `step` сохраняется благодаря `add`, ссылающейся на него. На этом механизме построены декораторы, рассматриваемые в следующей главе.
...
Чтение имени возможно из любой внешней области, а присваивание всегда создаёт **локальную** переменную. Решение принимается при компиляции функции и распространяется на всё её тело, поэтому `min += 1` завершается ошибкой не на присваивании, а на попытке прочитать локальную `min`, ещё не получившую значения.
...
Многословность здесь намеренная: функция, изменяющая глобальное состояние, перестаёт быть предсказуемой, и её результат зависит от числа предыдущих вызовов. `global` следует использовать только там, где без него нельзя обойтись.

### Оператор nonlocal (Python 3+)

`nonlocal` выполняет то же самое, но для объемлющей функции, а не для модуля. Это единственный способ позволить вложенной функции изменять переменную той функции, в которую она вложена, и на нём основаны счётчики и накопители внутри декораторов.
...
Две функции разделяют одну переменную `value`, существующую столько же, сколько они сами; извне до неё нельзя добраться иначе как через `get` и `set`.
...
Как только логика перестаёт умещаться в одну строку, ей следует дать имя через `def`. PEP 8 запрещает присваивать лямбду переменной: запись `f = lambda x: ...` ничем не лучше `def f(x): ...`, но при этом теряет имя в трассировке.
...
`zip` проходит по нескольким последовательностям параллельно и на каждом шаге возвращает кортеж, составленный из их элементов. Останавливается он по самой короткой, и лишние элементы длинных последовательностей теряются.
...
Код читают чаще, чем пишут, и единый стиль экономит время всем участникам. PEP 8 является официальным соглашением о том, как выглядит код на Python; его соблюдение проверяет любой линтер.
...
Пробелы вокруг операторов расставляются по приоритету, чтобы структура выражения была видна визуально. Тело `if` всегда переносится на новую строку, поскольку однострочную запись труднее заметить при беглом чтении, а также на ней нельзя поставить точку останова. Сравнение записывается в естественном порядке, с проверяемой переменной слева от значения, а отрицание принадлежности выражается оператором `not in`, а не через `not ... in`.
...
Ниже приведены те же четыре конструкции, записанные неправильно.
