...
Как только логика перестаёт умещаться в одну строку, ей лучше дать имя через `def`. PEP 8 запрещает присваивать лямбду переменной: запись `f = lambda x: ...` ничем не лучше `def f(x): ...`, но при этом теряет имя в трассировке.
