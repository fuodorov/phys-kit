...
Обращение к `super()` внутри `__setattr__` обязательно: `self.name = value` вызовет метод рекурсивно. По той же причине не следует переопределять `__getattribute__`, перехватывающий доступ ко **всем** атрибутам, а не только к отсутствующим.
...
`__repr__` следует определять у любого класса, существующего дольше пары строк, иначе при отладке объекты печатаются как `<__main__.Counter object at 0x7f3a…>`.
...
- Прежде чем строить иерархию наследования, следует проверить, не решается ли задача композицией: объект, **содержащий** другой объект, почти всегда проще объекта, унаследованного от него.
