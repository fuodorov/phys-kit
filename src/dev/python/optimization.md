...
обращение к ней приводит к `AttributeError`. Запись в чужой кадр по-прежнему возможна, однако требует более короткого кода.
...
Основная задача заключается в поиске места, к которому следует прикладывать усилия. Этому служат два правила.
