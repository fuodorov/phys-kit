...
Мы разобрались, как устроен Python и как писать на нём сопровождаемые программы. Пришло время заняться данными, ради которых физик вообще берёт в руки язык программирования, и начать придётся с инструментов, приспособленных считать быстро.

## NumPy

Здесь Python поначалу разочаровывает. Список из миллиона чисел занимает десятки мегабайт, а цикл по нему выполняется мучительно долго. Причины, разобранные в главе про объекты и память, сводятся к одному: каждое число в списке — это отдельный объект со своей обёрткой, разбросанный где-то в куче.

Решение, придуманное давно, называется **NumPy**. Идея в том, чтобы хранить числа так, как их хранит C, то есть подряд, одного типа, без обёрток, а операции над ними отдавать скомпилированному коду, работающему сразу над всем массивом. Выигрыш, получаемый на таком хранении, доходит до сотен раз. На нём и стоит весь научный стек: SciPy, pandas, scikit-learn и библиотеки машинного обучения построены на массивах NumPy.

С NumPy мы и начнём, затем перейдём к pandas, надстройке, приспособленной для работы с таблицами, а закончим визуализацией.
...
Всё держится на одном объекте, называемом [ndarray](https://docs.scipy.org/doc/numpy/reference/generated/numpy.ndarray.html).
...
Массив можно получить тремя путями: преобразовать из обычной структуры Python, сгенерировать встроенной функцией или прочитать с диска. Разберём все три способа, встречающиеся примерно одинаково часто.
...
Метод [reshape](https://docs.scipy.org/doc/numpy/reference/generated/numpy.reshape.html), меняющий форму массива без изменения самих данных, устроен так
...
Многомерный массив, развёрнутый в вектор, получается функцией [ravel](https://numpy.org/doc/stable/reference/generated/numpy.ravel.html)
...
В NumPy работает привычная индексация Python, включая отрицательные индексы и срезы, записываемые ровно так же, как для списка
...
**Замечание**: индексы и срезы, перечисляемые для многомерного массива, разделять квадратными скобками не нужно 
...
Срез массива в NumPy даёт представление на те же данные, а не их копию. Изменишь срез вроде `x[2:5]`, и изменится исходный массив. А вот отбор по маске или по списку индексов, показанный выше, всегда возвращает копию, и исходный массив остаётся нетронутым. Когда нужны собственные данные, не связанные с чужой памятью, есть метод `copy`.
...
встретится ниже прямо в выводе). Интересна здесь только структура, то есть текстовая
таблица с заголовком, где четыре числовых столбца и один строковый. Сделай такой
файл сам, а в первую строку помести имена столбцов, разделённые `, `:
...
Если хочется работать с настоящими ирисами, их отдаёт scikit-learn одной строкой:
...
`genfromtxt` с `names=True` возвращает структурированный массив, хранящий имена столбцов вместе с данными. Строку из него запрашивают по номеру, а колонку по названию.
...
При чтении из файла строки, стоящие в начале и в конце, пропускаются аргументами **skip_header** и **skip_footer**, а столбцы, нужные в расчёте, отбираются через **usecols**.
...
Операции в NumPy производятся прямо над векторами одинаковой размерности, без единого цикла, написанного руками.

Например, поэлементная разность, взятая между двумя векторами, выглядит так:
...
Broadcasting снимает требование одинаковой формы и разрешает арифметику над массивами разных, но согласованных между собой размерностей. Проще всего правило видно на векторе, умножаемом на число.
...
То есть чтобы broadcasting сработал, длины осей, отсчитываемых с конца, должны либо совпадать, либо одна из них должна быть равна единице.
...
Тот же трюк, повторённый со столбцами, так просто не пройдёт: вектор из четырёх элементов с матрицей по последней оси не согласуется.
...
Теперь эффективно посчитаем \\(\frac{1}{n} \sum\limits_{i=1}^n |x_i-y_i|\\) для каждой пары \\((x, y)\\), где \\(x\\) обозначает вектор признаков объекта из класса setosa, а \\(y\\) вектор признаков объекта из класса versicolor.
...
Почти у каждой сводной операции есть аргумент `axis`, задающий номер оси, вдоль которой она считается. Без него операция берёт весь массив целиком и возвращает одно число, а с ним сворачивает только указанную ось, оставляя остальные нетронутыми. На трёхмерном примере, приведённом ниже, это видно нагляднее всего.
...
Массивы, склеиваемые в один, обрабатывают функции **np.concatenate, np.hstack, np.vstack, np.dstack**, различающиеся только осью, вдоль которой идёт склейка.
...
Раньше NumPy молча обрезал значение, не помещавшееся в тип, и лишь предупреждал;
с версии 2.0 это ошибка. Тихое переполнение осталось только у явного приведения:
...
\\( 70000 - 65536 = 4464 \\). Ошибка, полученная таким путём, не падает
и ничего не печатает, поэтому тип массива и стоит выбирать осознанно.
...
Есть соблазн применять к массиву обычную функцию Python поэлементно. NumPy предлагает для данной цели несколько инструментов, и разница в скорости, измеренная между ними, поучительна.
...
Pandas читает данные, приводит их в порядок, считает по ним сводки и тут же рисует графики. Если NumPy даёт массив чисел, то pandas даёт таблицу с именованными колонками и индексом, размечающим строки.
...
Набор данных о «Титанике» — учебная классика, лежит в открытом доступе (например,
в [OpenML](https://www.openml.org/d/40945) под именем `titanic`). Разделитель здесь
важен. Файл идёт с табуляцией, а не с запятой, и `sep='\t'` обязателен.
Файл, скачанный из другого места, вполне может оказаться обычным CSV.
...
Начнём с сортировки и заодно увидим, что индекс, приписанный строке, сохраняется и перестаёт совпадать с её номером по порядку.
...
Если хочешь менять данные среза, не трогая основную таблицу, работай с копией, отвязанной от неё.
...
А если менять нужно саму основную таблицу, используй `loc`.
...
У `Series` и `DataFrame` много общих методов, работающих одинаково и там и там.
...
Одномерный срез датафрейма даёт `pd.Series`, колонку значений, снабжённую индексом.

Достать из `pd.Series` обычный `np.array` можно, но обычно не нужно: вместе с массивом, оторванным от таблицы, теряется индекс, а он в pandas и есть самое ценное.
...
Создаются они примерно так же, как `np.array`, только индекс, назначенный элементам, задаётся явно.
...
`Series`, полученная любым из способов, при необходимости разворачивается обратно в `DataFrame`:
...
Две таблицы, снятые с одной установки, сводятся в одну методом `join`, сопоставляющим строки по индексу.
...
Сам по себе `groupby` ничего не считает, а возвращает объект, который помнит разбиение и ждёт сводной операции.
...
Время в данных эксперимента заслуживает отдельного разговора. Показания приборов почти всегда приходят с меткой времени, проставленной системой сбора. Добавим в таблицу колонку с временем в формате Unix и посмотрим, что pandas умеет делать с ней, приведённой к своему типу.
...
Колонка, переведённая в `datetime`, сразу получает целый набор операций, недоступных обычному целому числу.
...
Время, поставленное индексом, открывает `resample`, пересчитывающий ряд на равномерную сетку заданного шага.
...
Скользящее окно (rolling) сглаживает шумный ряд. По каждой точке считается среднее (или сумма, или максимум) нескольких соседних. Первые значения, попавшие в неполное окно, остаются пустыми, потому что окну не из чего считать.
...
Скользящее окно дружит с группировкой: окно, взятое внутри каждой группы, считается независимо и не размазывает соседние группы друг по другу.
...
У строковых колонок есть аксессор `.str`, через который к каждому элементу применяются обычные строковые методы, выстраиваемые цепочкой. Ниже из полного имени пассажира вытаскивается личное имя: строку приводят к нижнему регистру, заменяют запятую, разрезают по точке и берут вторую часть, оставшуюся после разреза.
...
В реальных данных дырки есть всегда. Прибор не сработал, поле анкеты осталось пустым. Pandas обозначает такие места как `NaN` и даёт несколько способов с ними обойтись: строки, испорченные пропуском, выбрасывают целиком, заполняют подходящим значением или находят маской.
...
Когда готовой векторной операции не нашлось, остаётся `apply`, прогоняющий твою функцию по каждой строке (`axis=1`) или по каждому столбцу. Средство это последнее, потому что внутри работает обычный цикл Python со всеми издержками, разобранными в главе про производительность, и перед тем, как писать `apply`, поищи векторное решение ещё раз.
...
Быстрый график, построенный прямо из таблицы, — то, ради чего pandas часто и открывают. Метод `plot()` рисует ряд как есть, `resample("10s").mean().plot()` сначала усредняет его по десятисекундным интервалам и потому даёт кривую, очищенную от дрожания, а `hist()` строит гистограмму распределения.
