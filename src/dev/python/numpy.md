...
NumPy — основная библиотека для научных вычислений на Python: высокопроизводительный объект многомерного массива и инструменты для работы с такими массивами.

Сначала импортируй пакет `numpy`.
...
Массив NumPy — сетка значений одного типа, индексируемую кортежем неотрицательных целых чисел. Число измерений массива хранится в поле `ndim`, а форма (shape) — кортеж целых чисел с размерами вдоль каждого измерения.
...
Срезы. Как и списки Python, массивы NumPy можно нарезать срезами; поскольку массивы многомерны, срез указывается для каждого измерения.
...
Целочисленную индексацию можно смешивать со срезами, однако в результате получится массив меньшей размерности, чем исходный.
...
Два способа обратиться к средней строке массива: смешивание целочисленной индексации со срезами даёт массив меньшей размерности,
а одни только срезы сохраняют число измерений исходного массива.
...
Индексация целочисленными массивами. Срез всегда даёт подмассив исходного массива. Индексация целочисленными массивами позволяет собирать произвольные массивы из данных другого массива.
...
Булева индексация. Позволяет выбирать произвольные элементы массива; чаще всего применяется, чтобы отобрать элементы, удовлетворяющие условию.
...
Многие детали индексации здесь опущены; остальное — в документации.

### Типы данных

Каждый массив NumPy состоит из элементов одного типа; библиотека предоставляет большой набор числовых типов. При создании массива NumPy угадывает тип сам, но функции создания массивов обычно принимают необязательный аргумент, задающий тип явно.
...
Базовые математические функции работают с массивами поэлементно и доступны и как перегруженные операторы, и как функции модуля numpy:
...
В отличие от MATLAB, оператор `*` выполняет поэлементное умножение, а не матричное. Для скалярного произведения векторов, умножения вектора на матрицу и перемножения матриц служит функция dot — и как функция модуля numpy, и как метод объекта-массива.
...
Одна из самых востребованных функций для вычислений над массивами — `sum`.
...
Кроме вычислений часто нужно менять форму данных в массивах. Простейший пример — транспонирование матрицы через атрибут T объекта-массива.
...
Broadcasting (транслирование) — механизм, позволяющий NumPy выполнять арифметические операции над массивами разной формы. Типичный случай: есть маленький массив и большой, и маленький нужно использовать много раз в операции над большим.

Допустим, нужно прибавить постоянный вектор к каждой строке матрицы:
...
Это работает, но когда матрица `x` велика, явный цикл на Python медленный. Прибавление вектора v к каждой строке `x` равносильно тому, чтобы сформировать матрицу `vv`, вертикально уложив копии `v`, а затем поэлементно сложить `x` и `vv`.
...
Строка `y = x + v` работает, хотя у `x` форма `(4, 3)`, а у `v` форма `(3,)`: благодаря broadcasting она выполняется так, как если бы `v` имел форму `(4, 3)`, каждая его строка была копией `v`, а сложение шло поэлементно.
...
Другое изложение тех же правил — в [документации](http://docs.scipy.org/doc/numpy/user/basics.broadcasting.html).

Функции, поддерживающие broadcasting, называются универсальными; полный список — в [документации](http://docs.scipy.org/doc/numpy/reference/ufuncs.html#available-ufuncs).

Несколько применений broadcasting:
...
Broadcasting обычно делает код лаконичнее и быстрее; используй его везде, где возможно.

Этот обзор далеко не полон; остальное — в [справочнике NumPy](http://docs.scipy.org/doc/numpy/reference/).