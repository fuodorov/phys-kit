...
Библиотек, написанных для рисования графиков, в Python много, но физику обычно хватает трёх. Matplotlib отдаёт статическую картинку, вставленную потом в статью. Plotly добавляет интерактив, ценный при разборе сырых данных: наведение с точными числами, зум, много измерений сразу на одном полотне. HoloViews предлагает описывать данные вместо того, чтобы рисовать график.
...
`subplots` возвращает пару, где фигура `fig` — это весь холст с полями и общим заголовком, а `ax` — прямоугольник с координатной сеткой, отведённый под сам график. Рисуют в оси, сохраняют фигуру. Вызов `tight_layout` подбирает поля так, чтобы подписи, поставленные у осей, не срезались краем. `savefig` пишет файл, а `plt.show()` открывает окно, блокирующее скрипт до закрытия, и в программе нужно либо одно, либо другое.
...
Пока данные разбираешь сам, хочется подвести курсор и прочитать точное число, выделить рамкой кусок и увеличить, погасить лишнюю кривую щелчком по легенде. Это даёт [Plotly](https://plotly.com/python/), рисующий график в браузере поверх библиотеки plotly.js.

```bash
pip install plotly
```

Возьмём огибающую пучка, посчитанную вдоль тракта. Данные даёт уравнение Капчинского—Владимирского, выведенное в отдельной главе, а тракт собран из пяти соленоидов, расставленных через полметра.
...
Когда осей нужно больше трёх, берут параллельные координаты, где каждая переменная получает свою вертикальную ось, а один опыт превращается в ломаную, пересекающую все оси разом. Разыграем триста случайных настроек тракта и оценим каждую по среднеквадратичному отклонению радиуса от целевых четырёх миллиметров.
...
Тысяча строк по восемь чисел таблицей нечитаема, а на параллельных координатах читается. Там, где жгут линий сливается в узкую полосу, параметр решает дело, и в эту полосу обязательно попасть. Там, где линии размазаны по оси ровно, параметр на результат почти не влияет, и его отдают оптимизатору целиком. Хорошие сочетания видны, потому что тёмные ломаные идут пучком. В браузере оси перетаскиваются мышью, а по каждой выделяется диапазон, и на экране остаются одни прогоны, которые прошли отбор.

## HoloViews

Matplotlib рисует то, что ему велено, и потому заставляет расписывать каждый шаг, от подписи оси до заданного вручную цвета.

[HoloViews](https://holoviews.org) построен на принципе «аннотируй данные, а не рисуй график». Ты описываешь семантику данных, то есть что здесь аргумент, что значение и в каких они единицах, а библиотека сама выбирает представление. Один и тот же объект, собранный однажды, отрисовывается разными бэкендами: интерактивным `bokeh`, `matplotlib` или `plotly`.

```bash
pip install holoviews bokeh
```

Основа — элемент, контейнер с данными, знающий свой тип визуализации. `hv.Curve` задаёт непрерывную зависимость, `hv.Scatter` показывает дискретные измерения, `hv.Image` отображает величину на регулярной сетке. Измерениям задают человеческие подписи и единицы, попадающие потом на оси и во всплывающие подсказки.
...
В Jupyter объект, возвращённый последней строкой ячейки, рисуется сам, так что аналог `%matplotlib inline` не нужен. С бэкендом `bokeh` панорамирование, зум и подсказки есть сразу, и подозрительный выброс, замеченный на графике, разглядывается вблизи без перестроения картинки.

`hv.Points` внешне похож на `Scatter`, но семантика у него другая: обе координаты выступают независимыми измерениями, а точки просто живут на плоскости, как частицы на фазовой плоскости. Готовые элементы комбинируются операторами, из которых `+` кладёт графики рядом, а `*` накладывает их в одних осях.
...
В matplotlib скан по параметру превращается в цикл, склеивающий десяток подграфиков в одну фигуру, а `HoloMap` делает из того же параметра слайдер.
...
`HoloMap` считает все кадры заранее и держит их в памяти, зато сохраняется в самодостаточный HTML, который можно отправить коллеге. `DynamicMap` вычисляет кадр по движению слайдера, что нужно, когда параметр непрерывный или каждый расчёт дорог, но без запущенного ядра Python объект не покажется, и в статичный файл он не ложится.
