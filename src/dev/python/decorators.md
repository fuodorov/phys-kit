...
`functools.wraps` является декоратором, копирующим в обёртку метаданные оригинала (`__name__`, `__doc__`, `__module__`); ссылка на исходную функцию помещается в `__wrapped__`. При написании собственного декоратора над внутренней функцией всегда следует ставить `@functools.wraps(func)`.
...
- Настройки декоратора следует оформлять ключевыми аргументами, иначе `@deco` и `@deco(...)` невозможно различить.
