...
> Опирается на главы [«Инструменты Linux»](../linux/tools.md) и [«Внутреннее устройство Linux»](../linux/structure.md); `strace` и `perf` в книге только названы, поэтому разбираться с ними придётся по `man perf-record`, `man perf-report` и по схемам Брендана Грегга.
...
Эту часть книга не покрывает: рабочее место каждый настраивает под себя по документации выбранных инструментов.
